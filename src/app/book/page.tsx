...
export default function BookAppointment() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!shopResponse.ok) throw new Error(shopData.error || 'Failed to load shop details')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.email || !formData.phone || 
        !formData.date || !formData.timeSlot || formData.selectedServices.length === 0) {
      toast({
    ...
    try {
      ...
      toast({
        ...
        action: <ToastAction altText="Home" onClick={() => router.push("/")}>Go Home</ToastAction>
      })
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="container mx-auto px-4 py-8">
        <Button variant="ghost" className="mb-6 pl-0 text-blue-600" onClick={() => router.push("/")}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl text-blue-800">Book an Appointment</CardTitle>
                <CardDescription>
                  {shop ? `Fill in your details for ${shop.name}` : "Loading shop details..."}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-slate-500">Choose one or more services</p>
                    {isLoading ? (
                      <div className="space-y-4">
                        {[1, 2, 3].map((i) => (
          ...
          <div>
            <Card className="sticky top-6">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <h4 className="font-medium text-slate-700">Shop</h4>
                  {shop ? (
                    <>
                ...
                <div className="space-y-2">
                  <h4 className="font-medium text-slate-700">Selected Services</h4>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-slate-600">{formData.timeSlot || "No time selected"}</p>
                </div>
              ...
              <CardFooter className="bg-slate-50 border-t rounded-b-lg">
                <p className="text-sm text-slate-500 w-full text-center">You will pay at the shop after your service</p>
              </CardFooter>
            </Card>
          </div>
        </div>
      ...
      <footer className="bg-blue-800 text-white py-8 mt-20">
        <div className="container mx-auto px-4 text-center">
          <p className="text-blue-200">
            © {new Date().getFullYear()} {shop?.name || "Barber Shop"}. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
